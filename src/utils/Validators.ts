import { ValidatorsModel } from "../models/validators-model";

export const isEmailValid = (email: string): ValidatorsModel => {
...
export const isFullnameValid = (fullname: string): ValidatorsModel => {
...
export const isPhoneValid = (phone: string): ValidatorsModel => {
  if (!/^\d+$/.test(phone)) {
    return {
...
export const isSubjectValid = (subject: string): ValidatorsModel => {
  let subjectV = subject;
...
export const isMessageValid = (message: string): ValidatorsModel => {
  if (message === "") {
    return {
